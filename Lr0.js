...
*/
function generateTable(tracing) {
  let tablaDatos = [];
  //genera la tabla a partier de los sig y los no terminales
  const generarEstructura = () => {
    const tabla = { Estado: 0 };
    for (let i = 0; i < terminales.length; i++) {
      tabla[terminales[i]] = "";
    }
    tabla.$ = "";
    for (let j = 0; j < noTerminales.length; j++) {
      if (!noTerminales[j].includes("'")) {
        tabla[noTerminales[j]] = "";
      }
    }
    return tabla;
  };
  //Se agrega el estado 0
  let tabla0 = generarEstructura();
  tabla0.Estado = 0;
  tablaDatos.push(tabla0);

  //Agrega cada estado a una lista
  const agregarEstados = () => {
    for (let i = 0; i < tracing.length; i++) {
      let tabla = generarEstructura();
      tabla.Estado = tracing[i].destino;
      tablaDatos.push(tabla);
    }
  };
  //Agrega las transiciones que hay del origen al destino
  const estadosOrigen = () => {
    for (let i = 0; i < tablaDatos.length; i++) {
      for (let j = 0; j < tracing.length; j++) {
        if (tracing[j].origen === tablaDatos[i].Estado) {
          if (noTerminales.includes(tracing[j].transicion)) {
            tablaDatos[i][tracing[j].transicion] = tracing[j].destino;
          } else {
            tablaDatos[i][tracing[j].transicion] = `I${tracing[j].destino}`;
          }
        }
      }
      /*
      if (tracing[i].destino === 1) {
        copyTable = generarEstructura();
        copyTable.Estado = 1;
        copyTable.$ = "Acepted";
        tablaDatos.push(copyTable);
        flags.push(copyTable.Estado);
      }
      */
    }
  };

  //Agrega los estados de aceptacion
  const estadosDestino = () => {
    for (let i = 0; i < tablaDatos.length; i++) {
      for (let j = 0; j < tracing.length; j++) {
        if (tablaDatos[i].Estado === tracing[j].destino) {
          if (tablaDatos[i].Estado === 1) {
            tablaDatos[i].$ = "Acepted";
          }
          if (terminales.includes(tracing[j].transicion)) {
            tablaDatos[i][tracing[j].transicion] =
              tracing[j].EstadoAceptado === undefined
                ? ""
                : `R${tracing[j].EstadoAceptado}`;
          }
        }
      }
    }
...
let transiciones = tracing(estados);
console.log(transiciones);
//Donde se imprime la tabla
console.log("----------Tabla de transiciones SLR0-----------");
generateTable(transiciones);
